...
void ParticleFilter::init(double gps_x, double gps_y, double theta, double std[]) {
   ...
   */
      ...
      num_particles = 35;  // TODO: Set the number of particles
...
void ParticleFilter::dataAssociation(vector<LandmarkObs> predictions, 
  ...
  // see lesson 5, chapter 10 and 11
	try{
     
      for (unsigned int o = 0; o < t_observations.size(); o++) {
        double m_distance = 9999999999.0;	// large number, 4 times the sensor_range

        for (unsigned int p = 0; p < predictions.size(); p++) {
          double curr_dist = dist(t_observations[o].x, t_observations[o].y, predictions[p].x, predictions[p].y);

          if (curr_dist< m_distance){		// dataAssociation, search the shortest distance
            m_distance = curr_dist;		// always fit to the closest
            t_observations[o].id = predictions[p].id; // save best observation
          } 
        }
      }    
  	}
  ...
  try{
      ...
      for(int i=0; i<num_particles; i++){
        ...
        particles[i].weight = 1.0;		// reset weight to have a starting neutral number 1 to be multiplied
      	for (unsigned int o=0; o<t_observations.size(); o++) {
          double near_x = map_landmarks.landmark_list[t_observations[o].id - 1].x_f;
          double near_y = map_landmarks.landmark_list[t_observations[o].id - 1].y_f;
          
          double w = ( 1/(2*M_PI*std_landmark[0]*std_landmark[1])) * 
              exp( -( pow(t_observations[o].x-near_x,2) / (2*pow(std_landmark[0], 2))
                  + ( pow(t_observations[o].y-near_y,2) / (2*pow(std_landmark[1], 2))) ) );		// Formla from the lessons
